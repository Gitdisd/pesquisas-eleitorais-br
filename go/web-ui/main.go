...
func render() {
	...
	b.WriteString(<h2>Filtros</h2>)
	b.WriteString(<label>Turno <select id="round">)
	for _, n := range []int{1, 2} {
		selected := ""
		if n == view.round { selected = " selected" }
	...
	for _, n := range candidates {
		selected := ""
		if n == view.candidate { selected = " selected" }
		b.WriteString(<option value=" + esc(n) + " + selected + > + esc(n) + </option>)
	}
	b.WriteString(</select></label> )
	b.WriteString(<label>Geografia <select id="geo"><option value="ALL">Brasil / todas</option>)
	for _, n := range geos {
		selected := ""
		if n == view.geo { selected = " selected" }
		b.WriteString(<option value=" + esc(n) + " + selected + > + esc(n) + </option>)
	}
	b.WriteString(</select></label> )
	...
	for _, n := range institutes {
		selected := ""
		if n == view.institute { selected = " selected" }
		b.WriteString(<option value=" + esc(n) + " + selected + > + esc(n) + </option>)
	}
	b.WriteString(</select></label> )
	b.WriteString(<label>Busca <input id="query" value=" + esc(view.query) + " placeholder="instituto, cenário, candidato…"></label> )
	b.WriteString(<button id="reset">Limpar</button>)

	b.WriteString(<h2>Resumo</h2><ul>)
	...
	b.WriteString(<h2>Tendência</h2>)
	b.WriteString(chartSVG(rows))

	b.WriteString(<h2>Pesquisas</h2>)
	...
	for _, p := range rows {
		for _, c := range p.Candidates {
			if view.candidate != "ALL" && c.Name != view.candidate { continue }
	...
	if shown == 0 { b.WriteString(<tr><td colspan="6">Nenhuma pesquisa corresponde aos filtros.</td></tr>) }
	b.WriteString(</tbody></table>)

	b.WriteString(<details><summary>Metodologia e proveniência</summary>)
	if latest != nil {
	...
	b.WriteString(<p>Build: Go WebAssembly · GitHub Pages static artifact.</p>)

	root.Set("innerHTML", b.String())
	bind("round", func(v string) { view.round, _ = strconv.Atoi(v); render() })
	bind("candidate", func(v string) { view.candidate = v; render() })
	bind("geo", func(v string) { view.geo = v; render() })
	bind("institute", func(v string) { view.institute = v; render() })
	bind("query", func(v string) { view.query = v; render() })
	bindButton("reset", func() { view = state{round: 2, candidate: "Lula", geo: "ALL", institute: "ALL"}; render() })
}

func bind(id string, change func(string)) {
	el := js.Global().Get("document").Call("getElementById", id)
	if !el.Truthy() { return }
	fn := js.FuncOf(func(this js.Value, args []js.Value) any { change(el.Get("value").String()); return nil })
	keepAlive = append(keepAlive, fn)
	el.Call("addEventListener", "change", fn)
	if id == "query" { el.Call("addEventListener", "input", fn) }
}

func bindButton(id string, click func()) {
	el := js.Global().Get("document").Call("getElementById", id)
	if !el.Truthy() { return }
	fn := js.FuncOf(func(this js.Value, args []js.Value) any { click(); return nil })
	keepAlive = append(keepAlive, fn)
...
func filteredPolls() []Poll {
	...
	for _, p := range polls {
		if view.round != 0 && pollRound(p) != view.round { continue }
		if view.geo != "ALL" && p.Geo != view.geo { continue }
		if view.institute != "ALL" && p.Institute != view.institute { continue }
		if q != "" {
			...
			if !found {
				for _, c := range p.Candidates {
					if strings.Contains(strings.ToLower(c.Name), q) { found = true; break }
				}
			}
			if !found { continue }
		}
		if view.candidate != "ALL" {
			found := false
			for _, c := range p.Candidates { if c.Name == view.candidate { found = true; break } }
			if !found { continue }
		}
		out = append(out, p)
	}
	sort.Slice(out, func(i, j int) bool {
		if out[i].FieldworkEnd == out[j].FieldworkEnd { return out[i].Institute < out[j].Institute }
...
func pollRound(p Poll) int {
	if strings.Contains(p.Scenario, "2º turno") || strings.Contains(p.Scenario, "2° turno") { return 2 }
	return 1
}

func candidateNames() []string {
	set := map[string]bool{}
	for _, p := range polls { for _, c := range p.Candidates { set[c.Name] = true } }
	return sortedKeys(set)
}

func geoNames() []string {
	set := map[string]bool{}
	for _, p := range polls { if p.Geo != "" { set[p.Geo] = true } }
	return sortedKeys(set)
}

func instituteNames() []string {
	set := map[string]bool{}
	for _, p := range polls { if p.Institute != "" { set[p.Institute] = true } }
	return sortedKeys(set)
}
...
func chartSVG(rows []Poll) string {
	if len(rows) == 0 { return <p>Sem dados para o gráfico.</p> }
	...
	for _, p := range rows {
		for _, c := range p.Candidates {
			if view.candidate != "ALL" && c.Name != view.candidate { continue }
			x := 0.0
			if maxDay != minDay { x = float64(dateDistance(minDay, p.FieldworkEnd)) / float64(max(1, dateDistance(minDay, maxDay))) }
			y := float64(c.Percentage)
			px := float64(left) + x*float64(width-left-right)
			py := float64(height-bottom) - (y/60.0)*float64(height-top-bottom)
	...
	for n := range points { names = append(names, n) }
	sort.Strings(names)
	for i, n := range names {
