...
function renderTable() {
  const keys = state.round === 2 ? ['lula', 'flavio'] : CANDIDATES.map((c) => c.key)
  document.getElementById('thead').innerHTML = `<tr><th>Campo</th><th>Publicação</th><th>Instituto</th><th>Geo</th><th>TSE</th><th>N</th><th>Margem</th>${keys.map((k) => `<th>${CANDIDATES.find((c) => c.key === k).label}</th>`).join('')}<th>Fonte</th></tr>`<th>${CANDIDATES.find((c) => c.key === k).label}</th>`).join('')}<th>Fonte</th></tr>`
